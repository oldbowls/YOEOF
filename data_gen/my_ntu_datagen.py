...
def gendata(data_path, out_path, ignored_sample_path=None, benchmark='xview', part='eval'):
    ...
    for i, s in enumerate(tqdm(sample_name)):
        ...
        if data.shape[1] <= max_frame:
            continue
        fp = np.zeros((1, 3, max_frame + 1, num_joint, max_body_true), dtype=np.float32)
        for j in range(0, data.shape[1] - max_frame):
            fp[0, :, :, :, :] = data[:, j:j + max_frame + 1, :, :]
            # fp = pre_normalization(fp)
            name = '{}/{}_data_joint_{}_{}.npy'.format(out_path, part, s, str(j))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_folder', default=r'I:\ntu120\sub\v1')
