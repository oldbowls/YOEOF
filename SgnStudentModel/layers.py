...
def one_hot(spa):
    y = torch.arange(spa).unsqueeze(-1)  # (25,1) 生成0-24
    ...
    return y_onehot  # 生成(1,1,25,25)的矩阵
...
class SpatialNet(nn.Module):
    def __init__(self, num_joint, bias=True, dim=256, gcn_type='small'):
        ...
        # self.spa_embed = embed(num_joint, dim // 4, norm=False, bias=bias, num_joints=num_joint)  # 为什么这里的
        # 通道数是num_joint,因为这个是用来提取语义信息的
        # self.joint_embed = embed(3, dim // 4, norm=True, bias=bias, num_joints=num_joint)
        self.joint_embed = embed(3, dim // 2, norm=True, bias=bias, num_joints=num_joint)
    ...
    def forward(self, input):  # dif是速度
        bs, c, num_joints, step = input.shape
        pos = self.joint_embed(input)  # 编码输入的空间特征
        # spa1 = self.spa_embed(self.spa).repeat(bs, 1, 1, step)  # 编码节点的语义特征
        # dif = self.dif_embed(dif)
        # dy = pos + dif
        # dy = pos
        # input = torch.cat([dy, spa1], 1)
        input = pos
        g = self.compute_g1(input)  # compute self-attention graph  19.1%,得到一张图，用于聚合空间特征
        input = self.gcn(input, g)  # 图卷积
        output = input
...
class TempolNet(nn.Module):
    def __init__(self, seg, bias=True, dim=256):
        ...
        # self.tem_embed = embed(seg, dim, norm=False, bias=bias)#用来提取语义特征
        self.cnn = local(dim, dim, bias=bias, seg=seg)  #
    ...
    def __init__(self, dim=3, dim1=128, norm=True, bias=False, num_joints=25):
        ...
        if norm:
            self.cnn = nn.Sequential(
                norm_data(dim, num_joints=num_joints),  # 以帧为单位进行batch normalization
                cnn1x1(dim, 64, bias=bias),  # 1*1在channel扩张维度
                nn.ReLU(),
...
class local(nn.Module):
    def __init__(self, dim1=3, dim2=3, bias=False, seg=20):
        ...
        self.maxpool = nn.AdaptiveMaxPool2d((25, seg))  # 将joint维度变为25, t维度变为seg

        self.cnn1 = nn.Conv2d(dim1, dim1, kernel_size=(1, 3), padding=(0, 1), bias=bias)  # 1x3卷积在时间维度上进行卷积
        # self.cnn1 = nn.Conv2d(dim1, dim1, kernel_size=(1, 2), dilation=(1, 3), padding=(0, 1),
        #                       bias=bias)  # 1x3卷积变为1x4,从而取代maxpool,也能达到维度不变的目的。
        self.bn1 = nn.BatchNorm2d(dim1)
        self.relu = nn.ReLU()
        self.cnn2 = nn.Conv2d(dim1, dim2, kernel_size=1, bias=bias)  # 只是调整一下通道
...
class compute_g_spa(nn.Module):
    ...
    def forward(self, x1):
        g1 = self.g1(x1).permute(0, 3, 2, 1).contiguous()  # ->n,t,v,c
        g2 = self.g2(x1).permute(0, 3, 1, 2).contiguous()  # ->n,t,c,v
        g3 = g1.matmul(g2)  # 说白了，g1和g2分别是edge特征,
        g = self.softmax(g3)  # 矩阵乘法,->n,t,v,v
        return g  # ntvv
